...
export async function fixAccessibleEmoji(rc: RuleContext): Promise<vscode.CodeAction[]> {
  ...
  const result: AiFixResult = await runAIFix(AccessibleEmojiStrategy, ctx, callGpt, {
    log: true,
  ...
  if (result.kind === 'whole-element') {
    ...
    if (elementRanges) {
      ...
      console.warn('[A11Y][accessible-emoji] Could not find element ranges to apply whole-element fix.');
    }
  }
